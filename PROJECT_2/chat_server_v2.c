...
void decode_header(int client_fd,unsigned char*buf,int* opcode,int* mask,int *mask_offset,
...
void handle_public_message(int client_fd,char* decoded);
void handle_private_message(int client_fd,char* decoded);
        ...
        if (opcode == 0x1 && mask == 1) {
            ...
            if (!registered[client_fd]) {
                ...
                if (strncmp(decoded,"PUBLIC:",7)==0){
                    handle_public_message(client_fd,decoded);
                }
                else if (strncmp(decoded,"PRIVATE:",8)==0){
                    handle_private_message(client_fd,decoded);
                }
...
void decode_message(unsigned char* buf,int mask_offset,
                    int payload_len,char * decoded)
{
...
void handle_public_message(int client_fd,char* decoded)
{
    ...
    size_t max_msg_len=sizeof(message_with_name)-strlen(usernames[client_fd])-3;

    if(strlen(actual_msg)>max_msg_len){
        actual_msg[max_msg_len]='\0';
    }

    snprintf(message_with_name,sizeof(message_with_name),"%s: %s",usernames[client_fd],actual_msg);

    for(int fd=0;fd<10000;fd++){
        if(handshake_done[fd]==1 &&registered[fd]==1 && fd!=client_fd)
...
void handle_private_message(int client_fd,char* decoded)
{
    ...
    *colon='\0';
    ...
    for(int fd=0;fd<10000;fd++){
        if(handshake_done[fd]==1 &&registered[fd]==1 &&strcmp(usernames[fd],receiver)==0)
        {
            receiver_fd=fd;
            break;
        }
    ...
    if(receiver_fd==-1)
        ...
        size_t max_msg_len=sizeof(message_with_name)-strlen(usernames[client_fd])-12;

        if(strlen(actual_msg)>max_msg_len){
            actual_msg[max_msg_len]='\0';
        }

        snprintf(message_with_name,sizeof(message_with_name),"[Private] %s: %s",
                        usernames[client_fd],actual_msg);
